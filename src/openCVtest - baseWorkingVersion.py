...
def drawBird(x, y, birdScale, xc, yc, S):
    cutoff = 1
    axi.moveto(x*S+xc, y)
    arc(x*S+xc, y, birdScale, 0.7*birdScale, np.pi-cutoff, np.pi+cutoff, True)
    arc(x*S+xc + np.cos((np.pi/2)-cutoff)*birdScale*S*2, y, birdScale, 0.7*birdScale, np.pi-cutoff, np.pi+cutoff, True)
...
def drawVillage(tracedLine, i, S, xc, yc):
        ...
        while abs(tracedLine[i+c][1] - tracedLine[i+c+1][1]) < 0.5:
            ...
            axi.moveto((lx+rx)/2 - wt/2, hb)
            axi.lineto((lx+rx)/2 + wt/2, hb)
            axi.lineto((lx+rx)/2, hb - ht)
            axi.lineto((lx+rx)/2 - wt/2, hb)
...
def drawLandscape():
    ...
    for i in range(len(tracedLine)):
        if i > d:
            ...
            birdScale = 0.7
            if rand.random() < 0.02 and y > 2*birdScale:
                print("Drawing: Bird")
                drawBird(x, rand.uniform(10, (y-3*birdScale)*S+yc), birdScale, xc, yc, S)

            #generate flocks    
            if rand.random() < 0.005 and y > 2+3*birdScale:
                print("Drawing: Flock")
                stepSize = 7*S
                birdx = x
            ...
            if featureGen < 0.06:  # 6%
                ...
                d = drawLake(tracedLine, i, x, y, 8, 50, 1.8*S, S, xc, yc)
...
if not useVid:
    ...
    parser.add_argument('--input', help='', default='IMG_1996.jpg')
