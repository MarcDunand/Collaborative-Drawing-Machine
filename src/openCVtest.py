...
def getHandDrawnLine(vid, src, thresh):
    if useVid:
        ret, frame = vid.read()
        if not ret:
...
cv.createTrackbar('xc', 'Live Video Feed', xc, 2000, on_xstrackbar)
cv.createTrackbar('yc', 'Live Video Feed', yc, 1000, on_ystrackbar)
...
# cv.namedWindow('Working Area', cv.WINDOW_NORMAL)
# cv.resizeWindow('Working Area', 50, 50)
...
def drawTower(tracedLine, i, S, xc, yc):
    ...
    l = i - rand.randrange(1, 8)
    r = i + rand.randrange(1, 8)
    ...
    h = (tracedLine[r][1]*S + yc) - rand.uniform(15, 80)*S
    w = rx-lx
    ...
    if rand.random() < 0.3:
        ...
        ht = min((ry - h)/1.5, w*rand.uniform(0.7, 4))
...
def drawVillage(tracedLine, i, S, xc, yc):
        ...
        while abs(tracedLine[i+c][1] - tracedLine[i+c+1][1]) < 0.5:
            ...
            hb = tracedLine[i+c][1]*S+yc - rand.uniform(6, 16)*S
...
def drawLandscape():
    if useVid:
        handLine = getHandDrawnLine(vid, None, thresh)
    else:
        handLine = getHandDrawnLine(None, src, thresh)
    if handLine.size == 0:
    ...
    # [startX, startY] = handLine[0][0]
    # axi.moveto(startX*S+xc, startY*S+yc)
    # for elem in handLine:
    #     [x, y] = elem[0]
    ...
    for i in range(len(tracedLine)):
        ...
        if rand.random() < 0.1:
            ...
            drawStriation(tracedLine, i, x, y, 4, 70, S, xc, yc)
        ...
        if i > d:
            ...
            localW = 50
            if rand.random() < 1 and  i > localW + 2 and i < len(tracedLine)-localW - 2 and all(y <= coord[1] for coord in tracedLine[i-localW:i+localW]): # 4%
                print("Drawing: Tower")
                alignTest = True
            ...
            if featureGen < 0.06:  # 6%
                print("Drawing: Tree")
                alignTest = True
                d = drawTree(i, x*S + xc, y*S + yc, -50*S)
            elif featureGen < 0.09: # 3%
                print("Attempting: Village")
                alignTest = True
                d = drawVillage(tracedLine, i, S, xc, yc)
            elif featureGen < 0.13: # 4%
                print("Attempting: Lake")
                d = drawLake(tracedLine, i, x, y, 15, 120, 4*S, S, xc, yc)
